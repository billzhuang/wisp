// Package render defines the frontend seam — the boundary between the local
// shell session controller and however pixels (or terminal cells) get on screen.
//
// A Frontend reads shell output from a Controller, feeds it into a
...
type Controller interface {
	// Stdout is the shell output stream to feed into the terminal engine.
	Stdout() io.Reader
	// Input forwards locally-generated bytes to the shell's stdin.
	Input(p []byte) error
	// Resize informs the shell's PTY of a new window size.
